...
class HouseholdController {
	...
	async get(_req, _res, next) {
		try {
			...
			if (!(await getIsAuthorized(_req.auth?.payload.sub, data, ['owner', 'member']))) throw ThrowableError(buildErrorMessage(responseErrorCodes.NOT_AUTHORIZED, this.objectName, data), undefined, 401);
	...
	async update(_req, _res, next) {
      ...
      if (!(await getIsAuthorized(_req.auth?.payload.sub, data.id, ['owner']))) throw ThrowableError(buildErrorMessage(responseErrorCodes.NOT_AUTHORIZED, this.objectName, data.id), undefined, 401);
	...
	async delete(_req, _res, next) {
		try {
			const data = _req.params.id;
			householdValidator.identifierModel.validate(data);
      if (!(await getIsAuthorized(_req.auth?.payload.sub, data, ['owner']))) throw ThrowableError(buildErrorMessage(responseErrorCodes.NOT_AUTHORIZED, this.objectName, data), undefined, 401);
